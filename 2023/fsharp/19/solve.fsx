...
let rangeMapSize m = m |> Map.values |> Seq.map (fun (lb, ub) -> ub - lb - 1 |> int64) |> Seq.reduce (*)
let intersectRangeMaps m1 m2 =
    if m1 <> m2 && Map.forall (fun k r1 -> Map.find k m2 |> fun r2 -> (intersectRanges r1 r2).IsSome) m1 then
        let r = m1 |> Map.map (fun k r1 -> let r2 = Map.find k m2 in intersectRanges r1 r2 |> Option.get)
        Some r
    else None

let pairWith xs ys f =
    xs |> Seq.collect (fun x -> ys |> Seq.filter (snd >> (<>)x) |> Seq.choose (fun (s, y) -> 
        if Set.contains x s then None else f x y |> Option.map (fun z -> Set.add x s, z)))
...
let rec rangeMapsSize size acc xs =
    ...
    if size = 1 then
        ...
        let pairsIntersects = pairWith xs acc intersectRangeMaps |> Seq.distinctBy fst |> Seq.toList
        let sign = if size % 2 = 1 then 1L else -1L
        if pairsIntersects.IsEmpty then 0L else
