...
let races isPart2 =
    lines |> Seq.toList |> List.map (function
    | Match ".*: (.*)" [nums] -> 
        if isPart2 then (nums.Replace(" ", "")) else nums
        |> fun nums -> nums.Split(' ', System.StringSplitOptions.RemoveEmptyEntries) |> Array.map int64 |> Array.toList
    ) |> List.transpose |> List.map (fun xs -> xs[0], xs[1])
