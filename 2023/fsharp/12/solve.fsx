...
let pairCombNum xs ys = 
    ...
    let rec pairComb' = memoizeRec <| fun recf (xs, ys) ->
        ...
        | (i,g)::xs, y::ys ->
            ...
            | (i, g)::xs, y::ys when lg > y + 1 && validBlock -> 
                recf ((i+1, List.skip 1 g) :: xs) (y::ys) + recf ((i+y+1, List.skip (y+1) g) :: xs) (ys)
            | (i, g)::xs, y::ys when lg = y || (lg = y + 1 && validBlock) ->
                recf ((i+1, List.skip 1 g) :: xs) (y::ys) + recf xs ys
            | (i,g)::xs, ys ->
                recf ((i+1, List.skip 1 g) :: xs) ys
    pairComb' (xs, ys)
...
let unfold (xs: string, cs) =
    ...
    xs, cs
let part1() = parsed |> Seq.map (fun (xs, cs) -> lineCombNum xs cs) |> Seq.sum
printfn $"{part1()}"
let part2() = parsed |> Seq.map unfold |> Seq.map (fun (xs, cs) -> lineCombNum xs cs |> int64) |> Seq.sum
printfn $"{part2()}"
