...
let galaxyPairs =
    galaxies
    |> Seq.mapi (fun i g1 ->
        galaxies |> Seq.skip (i + 1)
        |> Seq.map (fun g2 -> g1, g2))
    |> Seq.collect id |> Seq.toList

let dist eSize (x1, y1) (x2, y2) =
    let dx = abs (x1 - x2) |> int64
    let dy = abs (y1 - y2) |> int64
    let ex = [min x1 x2 .. max x1 x2] |> Seq.filter (fun x -> Set.contains x emptyX) |> Seq.length |> int64
    let ey = [min y1 y2 .. max y1 y2] |> Seq.filter (fun y -> Set.contains y emptyY) |> Seq.length |> int64
