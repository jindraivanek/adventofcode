...
let parseInput (lines: string[]) =
    ...
    let starts =
        m
        |> Seq.filter (fun (c, _) -> c = 'S')
        |> Seq.map (snd >> fun p -> p, 0)
        |> List.ofSeq
...
let getGraph
    ...
    let neighF p =
        let moves =
            dirs |> List.map (posPlus p) |> List.filter (fun v -> Set.contains v m.Floors)
...
let pathsWithShortcut k diff start finish nodes (distMap: Map<_, _>) =
    ...
    nodes
    |> Seq.collect (fun u -> nodes |> Seq.map (fun v -> u, v))
    |> Seq.filter (fun (u, v) ->
        u <> v
        && dist u v <= k
        && distMap[start, u] + dist u v + distMap[v, finish] <= opt - diff)
    |> Seq.map (fun (u, v) ->
        printfn "%A %A" u, v
        u, v)
...
let dijkstra
    ...
    (neighF: 'n -> ('n * int * bool) list)
    heur
    (finishCond: 'n -> bool)
    resultMap
    =
    let pq =
        System.Collections.Generic.PriorityQueue<('n * (int * int * Set<_>)), int>()
    ...
    initNodes
    |> Seq.iter (fun (n, p) -> pq.Enqueue((n, (p, cheatN, set [])), p + heur n))
