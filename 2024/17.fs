...
let checksum (xs: byte seq) = xs |> Seq.indexed |> Seq.fold (fun c (i, x) -> c ^^^ (int64 x <<< (i % 64))) 0L

let instr opCode op c =
    let literal = int64 op
    let combo = match op with | 4uy -> c.A | 5uy -> c.B | 6uy -> c.C | x -> int64 x
    c.IP <- c.IP + 2
    match opCode with
    | 0uy -> c.A <- c.A / (1L <<< int32 combo)
    | 1uy -> c.B <- c.B ^^^ literal
    | 2uy -> c.B <- combo &&& 7L
    | 3uy -> if c.A <> 0L then c.IP <- int literal
    | 4uy -> c.B <- c.B ^^^ c.C
    | 5uy ->
        let x = combo &&& 7L
...
let part2 lines = 
    ...
    while not finish do
        if a%1000000L=0L then printfn "%i" a
        c.Output.Clear()
        c.OutputChecksum <- 0L
        c.IP <- 0
        c.A <- a
