...
let getMuls2 (s: string) =
    ...
    let matches = mulRegex.Matches s
    let instr = matches |> Seq.map (fun m -> m.Value, List.tail [ for g in m.Groups -> g.Value ])
    let mutable enabled = true
    instr
    |> Seq.filter (function
        | ("do()", _) -> enabled <- true; false
        | ("don't()", _) -> enabled <- false; false
        | _ -> enabled)
...
let part1 lines = 
   parseInput lines |> getMuls

let part2 lines = 
    parseInput lines |> getMuls2
