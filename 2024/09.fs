...
let parseInput (lines: string[]) =
    ...
    let blocks = allBlocks |> Seq.filter (fun (i, _) -> i%2 = 0) |> Seq.map (fun (i, x) -> i/2, x) |> Map.ofSeq
    blocks, allBlocks |> Seq.collect (fun (i, x) -> Seq.replicate x (if i%2 = 1 then None else (Some (i/2)))) |> Seq.toList
...
let moveBlocks wholeBlock (blocks: Map<int,int>) xs =
    ...
    let freeSpaces (arr: _[]) =
        ...
        while i < arr.Length do
            let x = Seq.initInfinite (fun j -> arr[i+j]) |> Seq.takeWhile ((=) None) |> Seq.length
            if x > 0 then i, x
            i <- i + (max x 1)
        }
        |> Seq.toArray
    ...
    let rs =
        if wholeBlock then
            blocks |> Map.toSeq |> Seq.sortDescending |> Seq.map (fun (x, l) -> List.findIndex ((=) (Some x)) xs, l, x) |> Seq.toList
        else xs |> List.indexed |> List.rev |> List.choose (fun (i, x) -> x |> Option.map (fun x -> i, x)) |> List.map (fun (i, x) -> i, 1, x)
    let arr = xs |> List.toArray
    let mutable freeSpacesMut = freeSpaces arr
    let findLeftMostFreeSpace k =
        freeSpacesMut
        |> Seq.tryFind (fun (_, x) -> x >= k)
    let rec loop rs =
        rs |> List.iter (fun (j, k, _) ->
