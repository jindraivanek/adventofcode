...
let run init step =
    (0, init)
    |> Seq.unfold (fun (i, s) -> step i s |> Option.map (fun x -> x, (i + 1, x)))
    |> Seq.append [ init ]
...
let private runDay' extraInputName (day: Day<'s1, 's2>) =
    ...
    | None -> printfn $"Input file for day %i{day.Day} does not exist"
...
module Seq =
    let mapWithState f init xs =
        ...
        xs
        |> Seq.map (fun x ->
            let (s, y) = f state x in
            state <- s
            y)

    let groupByAndMap f g xs =
        xs |> Seq.groupBy f |> Seq.map (fun (k, vs) -> k, g vs)
