...
let parseInput wide (lines: string[]) =
    ...
    let grid =
        ...
        |> Seq.map (
            Seq.collect (fun c ->
                if (c = '@' || c = 'O') && wide = 2 then
                    seq [ c; '.' ]
                else
                    Seq.replicate wide c)
        )
    ...
    let b =
        grid
        |> Seq.filter (fun (c, _) -> c = 'O')
        |> Seq.map (snd >> fun p -> set [ p; posPlus p ((wide - 1, 0)) ])
        |> set
...
let boxesMap (state: State) =
    state.Boxes
    |> Seq.collect (fun bs -> bs |> Seq.map (fun b -> b, bs))
    |> Map.ofSeq
...
let moveBox b d state =
    ...
    let rec loop acc =
        ...
        let movedBoxes =
            bs
            |> Seq.choose (fun b -> boxesMap |> Map.tryFind b)
            |> Seq.filter (fun x -> x <> b)
            |> set

        if Set.intersect bs state.Walls |> Set.isEmpty |> not then
            None
        elif (movedBoxes - acc) <> Set.empty then
            loop (acc + movedBoxes)
        else
            Some acc
    ...
    movedBoxes
    |> Option.map (fun movedBoxes ->
        { state with
            Boxes =
                state.Boxes - movedBoxes
                |> Set.union (movedBoxes |> Set.map (Set.map (posPlus d))) })
...
let part2 =
    { Init = parseInput 2
      ...
      Result =
        (fun s ->
            s.Boxes
            |> Seq.map (Seq.sort >> Seq.head)
            |> Seq.sumBy (fun (x, y) -> 100 * y + x)
            |> string) }
