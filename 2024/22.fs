...
let secretSeq2 x = secretSeq x |> Seq.map (fun x -> string x |> Seq.last |> string |> int)

let secretComb init = 
    secretSeq2 init |> Seq.take 2001 |> Seq.windowed 5
    |> Seq.map (fun x -> 
        let d = (x[1] - x[0]) * 1000000 + (x[2] - x[1]) * 10000 + (x[3] - x[2]) * 100 + (x[4] - x[3])
        d, (init, x[4]))

let part2 lines = 
    parseInput lines |> Seq.collect secretComb
    |> Seq.groupBy fst |> Seq.sortBy fst |> Seq.map (fun (k, g) -> 
