...
let parseInput (lines: string[]) =
    ...
    |> Seq.map (fun s ->
        ...
        |> Array.map int64
        |> fun xs -> xs[0], Seq.tail xs |> Seq.toList)
...
let checkOperators2 x xs =
    let bruteforceCheck target xs =
        let rec loop acc x xs =
            ...
            match xs with
            | [] when x = target -> failwithf "BRUTE FAIL: %s = %A" acc target
            | a :: xs ->
                loop $"{acc} + {a}" (x + a) xs
                || loop $"{acc} * {a}" (mulWithAcc a) xs
                || loop $"{acc} || {a}" ((string x + string a) |> int64) xs
            | _ -> false
    ...
    let rec loop x xs =
        ...
        let numSplit a =
            ...
            [ s.Length - 1 .. -1 .. 1 ]
            |> List.map (fun i -> int64 s[0 .. i - 1], int64 s[i..])
