...
type State =
    { Floor: Set<int * int>
      ...
      MapSize: int * int
      Visited: Set<int * int>
      VisitedRoute: Set<(int * int) * int>
      LoopObstacles: Set<int * int>
      Sim: Set<(int * int) * int>
      IsLoop: bool }
...
let isInMap' (x, y) s =
    x >= 0 && x < fst s.MapSize && y >= 0 && y < snd s.MapSize

let isInMap s = isInMap' s.Guard s

let turnRight s =
    { s with
        GuardDir = (s.GuardDir + 1) % 4 }

let step s =
    let g2 = posPlus s.Guard dirs[s.GuardDir]

    let s2 =
        if s.Wall |> Set.contains g2 then
            turnRight s
        else
            { s with
                Guard = g2
    ...
    { s2 with
        VisitedRoute = Set.add (s2.Guard, s2.GuardDir) s.VisitedRoute }

let part1 s =
    let s2 = step s
    if isInMap s2 then Some s2 else None
...
let sim =
    ...
    <| fun extraWall ->
        runFinishCond
            { initState with
                Wall = Set.add extraWall initState.Wall }
            (fun s -> isInLoop s || not (isInMap s))
            step
        |> Seq.toList
        ...
        if s.Guard <> s2.Guard then
            ...
            let o =
                if isInLoop (sim |> Seq.last) then
                    { s2 with
                        LoopObstacles = Set.add s2.Guard s.LoopObstacles
                        IsLoop = true }
                else
                    { s2 with IsLoop = false }

            { o with
                Sim = sim |> List.map (fun s -> s.Guard, s.GuardDir) |> Set.ofList }
        else
            s2
